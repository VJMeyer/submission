...
\href{https://www.lesswrong.com/posts/mJ5oNYnkYrd4sD5uE/clarifying-some-key-hypotheses-in-ai-alignment\#Agentive_AGI_}{\includegraphics[width=1.1\linewidth,height=\textheight,keepaspectratio]{images/hypotheses_diagram.pdf}}
...
\href{https://arxiv.org/pdf/2206.09360\#page=10.75}{\includegraphics[width=0.35\linewidth,height=\textheight,keepaspectratio]{images/mtair-qual-map.jpg}}
...
A comprehensive review\footnote{For a comprehensive exploration of how
  this thesis could evolve into a full research program, see Appendix K:
  From Prototype to Platform. The technical challenges and
  methodological innovations required for scaling AMTAIR are detailed
  there, along with concrete pathways for community development.} would
fill volumes, so let me provide a guided tour of the territories most
relevant to AMTAIR's mission.
...
The heart of AMTAIR\footnote{The development of effective prompts
  required extensive empirical refinement.

  Appendix L: Prompt Engineering - The Hidden Art documents this
  journey, revealing both the sophistication required for successful
  extraction and the brittleness of current approaches.} beats with a
two-stage rhythm: structure, then probability. This separation, which
initially seemed like an implementation detail, revealed itself as
fundamental to the extraction challenge.
...
success\footnote{3}. We need systematic validation that honestly
assesses both capabilities and limitations.
...
Several research directions\footnote{4} could enhance AMTAIR's
...
This thesis has demonstrated feasibility, but the true test lies in
building the future sketched in these pages. The
appendices---particularly the research roadmap (Appendix K), the prompt
engineering documentation (Appendix L), and the validation frameworks
(Appendix M)---transform abstract possibilities into concrete research
agendas. I invite readers to see this not as a completed work but as an
opening move in a larger game whose stakes could not be higher.
...
\chapter*{Appendix K: From Prototype to Platform: A Research Program
Roadmap}\label{sec-appendix-roadmap}
\addcontentsline{toc}{chapter}{Appendix K: From Prototype to Platform: A
Research Program Roadmap}

\markboth{Appendix K: From Prototype to Platform: A Research Program
Roadmap}{Appendix K: From Prototype to Platform: A Research Program
Roadmap}

\subsection*{K.1 The Arc of Scientific Progress}\label{sec-arc-progress}
\addcontentsline{toc}{subsection}{K.1 The Arc of Scientific Progress}

Every significant scientific tool begins as a crude prototype in
someone's lab---or in our case, someone's Jupyter notebook. The
microscope started as ground glass held between brass plates. The
telescope began with Galileo squinting through wavering lenses. AMTAIR,
in its current incarnation, occupies a similar position: functional
enough to demonstrate possibility, rough enough to inspire imagination
about what it could become.

This appendix sketches that becoming. Not as idle speculation but as a
concrete research program that could transform how humanity reasons
collectively about existential risks. The gap between current
implementation and envisioned platform isn't a failing---it's an
opportunity space rich with research questions, engineering challenges,
and potential for impact.

\subsection*{K.2 Technical Evolution: From Scripts to
Systems}\label{sec-technical-evolution}
\addcontentsline{toc}{subsection}{K.2 Technical Evolution: From Scripts
to Systems}

The current AMTAIR implementation resembles a gifted graduate
student---capable of impressive feats when everything goes right, prone
to puzzling failures when encountering the unexpected. Transforming this
into a robust system requires addressing several technical frontiers:

\textbf{Extraction Engine Maturation}

The two-stage extraction pipeline (ArgDown → BayesDown) represents a key
insight, but its current implementation barely scratches the surface.
Future development should explore:

\begin{itemize}
\item
  \textbf{Fine-tuned Models}: Rather than relying on general-purpose
  LLMs with clever prompting, we could fine-tune models specifically for
  argument extraction. Imagine a model trained on thousands of
  argument-structure pairs, learning the subtle patterns that
  distinguish causal claims from mere correlations, conditional
  relationships from simple associations.
\item
  \textbf{Multi-Modal Understanding}: Arguments don't live in text
  alone. Graphs, equations, diagrams---all carry crucial information.
  Next-generation extractors should parse a paper's Figure 3 showing
  feedback loops as naturally as they parse its prose.
\item
  \textbf{Confidence-Aware Extraction}: Current systems extract with
  binary certainty---either they find a relationship or they don't.
  Mature systems should express uncertainty about their extractions,
  flagging ambiguous passages for human review rather than guessing
  silently.
\end{itemize}

\textbf{Scaling Infrastructure}

Processing a single paper in minutes is impressive. Processing the
entire AI safety literature in hours would be transformative. This
requires:

\begin{itemize}
\item
  \textbf{Distributed Processing}: Extraction tasks naturally
  parallelize. A coordinated fleet of extraction workers could process
  hundreds of documents simultaneously, building comprehensive models of
  entire research communities' thinking.
\item
  \textbf{Incremental Updates}: As new papers appear daily, models
  should evolve continuously rather than requiring full reconstruction.
  This demands clever data structures that support efficient updates
  while maintaining consistency.
\item
  \textbf{Caching and Optimization}: Many arguments share common
  substructures. Intelligent caching could dramatically reduce
  computation by recognizing when we're re-extracting variations on
  familiar themes.
\end{itemize}

\subsection*{K.3 Methodological Innovations: Beyond
Extraction}\label{sec-methodological-innovations}
\addcontentsline{toc}{subsection}{K.3 Methodological Innovations: Beyond
Extraction}

The most exciting developments may come not from improving what AMTAIR
does but from expanding what it attempts:

\textbf{Argument Evolution Tracking}

Ideas don't spring forth fully formed---they evolve through academic
conversation. Future systems should:
...
This temporal dimension transforms static extractions into living
intellectual history.

\textbf{Community Model Synthesis}

Individual papers offer perspectives; research communities build
consensus (or reveal persistent disagreements). Advanced systems could:
...
As these tools gain influence, incentives for manipulation emerge.
...
\subsection*{K.4 Interface Revolutions: Making Models
Meaningful}\label{sec-interface-revolutions}
\addcontentsline{toc}{subsection}{K.4 Interface Revolutions: Making
Models Meaningful}

The most sophisticated extraction means nothing if stakeholders can't
engage meaningfully with results. Interface innovation deserves equal
priority with algorithmic advancement:

\textbf{Stakeholder-Specific Views}

A Pentagon strategist, an AI safety researcher, and a congressional
staffer need different things from the same model:

\begin{itemize}
\tightlist
\item
  \textbf{Executive Dashboards}: High-level risk assessments, key
  uncertainties, decision-relevant parameters
\item
  \textbf{Research Workbenches}: Full model access, sensitivity analysis
  tools, comparison capabilities
\item
  \textbf{Policy Interfaces}: Intervention testing, robustness checking,
  implementation pathway analysis
\item
  \textbf{Public Portals}: Educational simplifications, interactive
  explorations, transparent methodology
\end{itemize}

\textbf{Collaborative Modeling Environments}
...
\tightlist
\item
  Real-time collaborative model editing (think Google Docs for Bayesian
  networks)
\item
...
\item
  Enable ``model-backed'' writing where claims link directly to formal
  justifications
\end{itemize}

\subsection*{K.5 Institutional Architecture: Embedding in
Reality}\label{sec-institutional-architecture}
\addcontentsline{toc}{subsection}{K.5 Institutional Architecture:
Embedding in Reality}

Technology alone doesn't create change---institutions do. AMTAIR's path
to impact requires careful consideration of organizational realities:

\textbf{Pilot Program Design}

Strategic initial deployments could demonstrate value while revealing
challenges:

\begin{itemize}
\tightlist
\item
  \textbf{Think Tank Integration}: Partner with organizations like FHI,
  CSER, or GovAI to model their research portfolios, creating internal
  tools before public platforms
\item
  \textbf{Government Pilots}: Work with tech-forward agencies to
  prototype decision support systems, learning regulatory constraints
  early
\item
  \textbf{Academic Collaborations}: Integrate with journal submission
  systems, enabling authors to submit formal models alongside papers
\end{itemize}
...
These questions demand thoughtful institutional design, not just
technical solutions.

\subsection*{K.6 The Ecosystem Vision}\label{sec-ecosystem-vision}
\addcontentsline{toc}{subsection}{K.6 The Ecosystem Vision}

The ultimate goal isn't a better extraction tool---it's transformed
epistemic infrastructure for navigating existential risks. Imagine:

\textbf{Research Transformation}
...
This ecosystem doesn't emerge spontaneously---it requires coordinated
development across technical, institutional, and social dimensions.

\subsection*{K.7 Critical Challenges and Honest
Uncertainties}\label{sec-critical-challenges}
\addcontentsline{toc}{subsection}{K.7 Critical Challenges and Honest
Uncertainties}

Intellectual honesty demands acknowledging deep challenges that may
resist solution:

\textbf{The Formalization-Insight Tradeoff}

Something is always lost in translation from rich prose to formal
models. The question is whether what's gained exceeds what's lost. This
tradeoff may vary by domain:
...
Research must explore these boundaries empirically rather than assuming
universal applicability.

\textbf{Gaming and Manipulation}
...
Solutions require both technical robustness and institutional
resilience.

\textbf{Technocratic Risks}

Tools that appear to objectify subjective judgments risk creating false
authority:
...
Avoiding these pitfalls requires careful design emphasizing models as
tools for thinking, not substitutes for judgment.

\subsection*{K.8 A Decade Hence: Success
Scenarios}\label{sec-success-scenarios}
\addcontentsline{toc}{subsection}{K.8 A Decade Hence: Success Scenarios}

Ten years from now, what would success look like? Let me paint three
scenarios:

\textbf{The Conservative Success}: AMTAIR-derived tools become standard
in AI safety research. Papers routinely include formal models.
Disagreements focus on parameters rather than talking past each other.
Policy discussions reference shared models. Progress is incremental but
real.

\textbf{The Transformative Success}: Formal modeling revolutionizes how
humanity reasons about complex risks. International agreements rest on
explicit shared models. Public understanding of AI risk dramatically
improves through interactive visualizations. Coordination failures that
seemed intractable dissolve under shared epistemic infrastructure.

\textbf{The Pivotal Success}: At a crucial moment---perhaps during rapid
AI capability advancement---formal models enable coordination that
prevents catastrophe. The ability to quickly synthesize expert
knowledge, test interventions, and achieve consensus across stakeholders
makes the difference between successful navigation and disaster.

Each scenario justifies the research investment. Together, they suggest
the profound importance of building these capabilities now, before
they're desperately needed.

\subsection*{K.9 The Call to Build}\label{sec-call-to-build}
\addcontentsline{toc}{subsection}{K.9 The Call to Build}

This appendix has sketched a research program spanning computer science,
policy studies, institutional design, and philosophy. No single team can
build this alone. The work requires:

\begin{itemize}
\tightlist
\item
  \textbf{Technical pioneers} pushing extraction and modeling
  capabilities
\item
  \textbf{Interface designers} making complexity comprehensible
\item
  \textbf{Institutional architects} embedding tools in decision
  processes
\item
  \textbf{Domain experts} validating and extending models
\item
  \textbf{Critics and red-teamers} identifying failures before they
  matter
\end{itemize}

The code is open. The vision is shared. The need is urgent. What remains
is the building.

\chapter*{Appendix L: Prompt Engineering - The Hidden
Art}\label{sec-appendix-prompts-hidden}
\addcontentsline{toc}{chapter}{Appendix L: Prompt Engineering - The
Hidden Art}

\markboth{Appendix L: Prompt Engineering - The Hidden Art}{Appendix L:
Prompt Engineering - The Hidden Art}

\subsection*{L.1 The Unreasonable Effectiveness of Good
Prompts}\label{sec-unreasonable-prompts}
\addcontentsline{toc}{subsection}{L.1 The Unreasonable Effectiveness of
Good Prompts}

Behind AMTAIR's seemingly magical ability to transform prose into formal
models lies a more mundane reality: hundreds of hours spent crafting,
testing, and refining prompts. This appendix pulls back the curtain on
this process---partly as documentation, partly as cautionary tale about
the gap between demo and deployment.

Prompt engineering for argument extraction resembles teaching a
brilliant but literal-minded student. The model possesses vast knowledge
and impressive reasoning capabilities, yet lacks the contextual
understanding humans take for granted. Success requires anticipating
every way instructions might be misinterpreted, then preemptively
clarifying.

\subsection*{L.2 The Evolution of Extraction
Prompts}\label{sec-prompt-evolution}
\addcontentsline{toc}{subsection}{L.2 The Evolution of Extraction
Prompts}

The path from naive first attempts to functional prompts tells a story
of gradual enlightenment punctuated by frustrating failures:

\textbf{Generation 1: The Naive Optimist}

\begin{verbatim}
Extract the argument structure from this text as a directed graph.
\end{verbatim}

Result: Complete chaos. The model returned everything from flowcharts to
philosophical musings about the nature of arguments. Occasionally, by
pure chance, something resembling ArgDown would emerge.

\textbf{Generation 2: The Overspecifier}
...
Better, but the model began inventing nodes to fill out what it thought
should be complete arguments. Worse, it couldn't distinguish causal
claims from logical support, mixing deduction with empirical
relationships.

\textbf{Generation 3: The Pattern Teacher}
...
Progress! The model began producing recognizable ArgDown. But new
problems emerged: inconsistent node naming, difficulty with implicit
relationships, confusion about what constituted a ``claim'' versus
background information.

\textbf{Generation 4: The Philosophical Guide}

The breakthrough came from stopping trying to specify mechanical rules
and instead explaining the deeper purpose:
...
This anthropomorphic framing, combined with specific technical
instructions, finally yielded consistent, high-quality extractions.

\subsection*{L.3 The Anatomy of Effective
Prompts}\label{sec-prompt-anatomy}
\addcontentsline{toc}{subsection}{L.3 The Anatomy of Effective Prompts}

Effective extraction prompts share several characteristics discovered
through painful trial and error:

\textbf{Role Establishment}: Beginning with ``You are\ldots{}'' frames
the task appropriately. The model performs better when given a coherent
identity aligned with the task.

\textbf{Purpose Context}: Explaining why the task matters improves
performance. ``You are extracting arguments to help coordination''
yields better results than ``extract arguments.''

\textbf{Thinking Process}: Encouraging step-by-step reasoning
dramatically improves quality. The
\texttt{\textless{}analysis\textgreater{}},
\texttt{\textless{}variable\_identification\textgreater{}},
\texttt{\textless{}causal\_structure\textgreater{}} tags don't just
organize output---they enforce systematic thinking.

\textbf{Concrete Examples}: Showing desired output format with examples
prevents creative reinterpretation. The rain-sprinkler-grass example
anchors understanding.

\textbf{Error Prevention}: Explicitly stating common mistakes (``Don't
invent claims not in the text'') preempts frequent failures.

\textbf{Validation Instructions}: Asking the model to check its own work
catches many errors before they propagate.

\subsection*{L.4 The Probability Extraction
Challenge}\label{sec-probability-challenge}
\addcontentsline{toc}{subsection}{L.4 The Probability Extraction
Challenge}

Extracting structure proved easier than extracting probabilities.
Language models can identify relationships reasonably well but struggle
with numerical precision:

\textbf{The Calibration Problem}: When text says ``highly likely,'' what
probability should be extracted? Initial attempts using fixed mappings
(likely = 0.7) produced nonsensical results when authors used terms
idiosyncratically.

\textbf{The Consistency Challenge}: Probabilities must sum correctly,
respect logical constraints, and maintain coherence across related
estimates. Models frequently violate these constraints when extracting
piecemeal.

\textbf{The Implicit Estimation Issue}: Authors rarely state all
required probabilities explicitly. The model must infer reasonable
values while maintaining uncertainty about these inferences.

The solution involved separating probability extraction into its own
phase with specialized prompts that emphasize consistency checking and
uncertainty acknowledgment.

\subsection*{L.5 Prompt Engineering as Empirical
Science}\label{sec-prompt-science}
\addcontentsline{toc}{subsection}{L.5 Prompt Engineering as Empirical
Science}

Developing effective prompts requires rigorous empirical methodology:

\textbf{Test Set Construction}: We assembled diverse
arguments---technical papers, blog posts, policy documents---to ensure
prompts generalized beyond initial examples.

\textbf{Ablation Studies}: Systematically removing prompt components
revealed which elements truly mattered. Surprisingly, elaborate
technical specifications often hurt more than helped.

\textbf{Failure Analysis}: Cataloging extraction errors revealed
patterns. Certain linguistic constructions reliably confused the model,
leading to targeted prompt improvements.

\textbf{Version Control}: Prompts evolved through dozens of iterations.
Maintaining version history proved essential for understanding which
changes helped versus hurt.

\subsection*{L.6 The Prompt Engineering
Toolkit}\label{sec-prompt-toolkit}
\addcontentsline{toc}{subsection}{L.6 The Prompt Engineering Toolkit}

Several techniques proved invaluable:

\textbf{Chain-of-Thought Prompting}: Forcing explicit reasoning steps
improves extraction quality dramatically. The model must show its work,
making errors visible and correctable.

\textbf{Role-Playing}: Having the model adopt specific personas (``You
are a careful academic'') improves consistency and appropriate
conservatism in claims.

\textbf{Negative Examples}: Showing what NOT to do often works better
than endless positive specifications. ``Don't hallucinate nodes'' beats
complex instructions about fidelity.

\textbf{Iterative Refinement}: Using model outputs to improve prompts
creates a virtuous cycle. Each failure suggests prompt improvements.

\textbf{Temperature Tuning}: Lower temperatures improve consistency but
may miss creative interpretations. We settled on 0.3 for structure, 0.5
for probabilities.

\subsection*{L.7 Lessons for Future Systems}\label{sec-prompt-lessons}
\addcontentsline{toc}{subsection}{L.7 Lessons for Future Systems}

The prompt engineering journey offers broader lessons:

\textbf{Brittle Brilliance}: Current models exhibit stunning
capabilities wrapped in frustrating fragility. Small prompt changes
cause dramatic performance shifts.

\textbf{Hidden Complexity}: What seems simple to humans (extract the
argument) requires extensive specification for machines. The prompt
complexity reflects genuine task difficulty.

\textbf{Moving Target}: As models evolve, optimal prompts change.
GPT-4's best prompts differ from Claude's. Future models may require
fundamental reconceptualization.

\textbf{Human-AI Partnership}: The best results come from prompts that
leverage model strengths while acknowledging limitations. Pure
automation remains inferior to human-in-the-loop approaches.

\subsection*{L.8 Toward Prompt-Free Extraction}\label{sec-prompt-free}
\addcontentsline{toc}{subsection}{L.8 Toward Prompt-Free Extraction}

The current reliance on carefully crafted prompts represents a
limitation, not a feature. Future research should explore:

\begin{itemize}
\tightlist
\item
  \textbf{Fine-tuned Models}: Training specifically for argument
  extraction could eliminate prompt sensitivity
\item
  \textbf{Learned Prompting}: Systems that automatically discover
  optimal prompts for new domains
\item
  \textbf{Structured Interfaces}: Moving beyond text to specialized
  input/output formats
\item
  \textbf{Verification Systems}: Automated prompt quality assessment and
  improvement
\end{itemize}

Until these advances materialize, prompt engineering remains the hidden
art enabling AMTAIR's visible magic.

\chapter*{Appendix M: The Validation Frontier - Measuring Truth in
Argument Extraction}\label{sec-appendix-validation-frontier}
\addcontentsline{toc}{chapter}{Appendix M: The Validation Frontier -
Measuring Truth in Argument Extraction}

\markboth{Appendix M: The Validation Frontier - Measuring Truth in
Argument Extraction}{Appendix M: The Validation Frontier - Measuring
Truth in Argument Extraction}

\subsection*{M.1 The Epistemological
Puzzle}\label{sec-epistemological-puzzle}
\addcontentsline{toc}{subsection}{M.1 The Epistemological Puzzle}

How do you measure the correctness of something that has no single
correct answer? This question haunts AMTAIR's validation efforts. Unlike
named entity recognition (where ``Apple Inc.'' is unambiguously an
organization) or sentiment analysis (where ground truth can be
established through annotation), argument extraction operates in a space
where reasonable experts disagree not just on details but on fundamental
structure.

This appendix grapples with this challenge, documenting both what we
learned and what remains unresolved. The validation journey revealed as
much about the nature of arguments themselves as about our system's
capability to extract them.

\subsection*{M.2 The Chimera of Ground
Truth}\label{sec-ground-truth-chimera}
\addcontentsline{toc}{subsection}{M.2 The Chimera of Ground Truth}

Our first instinct---create a benchmark dataset with ``correct''
extractions---crashed against reality. Consider a seemingly simple
passage:

``Advanced AI systems will likely pursue instrumental goals like
self-preservation and resource acquisition. Without careful alignment,
these drives could lead to outcomes catastrophic for humanity.''
...
Both are reasonable. Expert A sees a simple causal chain. Expert B
identifies multiple contributing factors. Who is correct? The question
misunderstands the nature of argument extraction.

\subsection*{M.3 Embracing Legitimate
Variation}\label{sec-legitimate-variation}
\addcontentsline{toc}{subsection}{M.3 Embracing Legitimate Variation}

Rather than seeking single correct extractions, we developed methods for
characterizing and measuring legitimate variation:

\textbf{Structural Similarity Metrics}: Borrowing from bioinformatics,
we adapted tree edit distance algorithms to measure how different two
extractions are structurally. Small edit distances suggest agreement on
main relationships despite surface differences.

\textbf{Semantic Overlap Assessment}: Using embedding similarities, we
can determine whether differently-named nodes represent the same
concepts. ``Catastrophic\_Outcomes'' and ``Catastrophic\_Risk'' are
structurally different but semantically similar.

\textbf{Core-Periphery Analysis}: Experts tend to agree on central
claims while varying on peripheral details. By identifying
highly-connected nodes, we can focus validation on what matters most.

\textbf{Causal Path Preservation}: Different extractions that preserve
the same causal paths (A→B→C) are functionally equivalent for many
purposes, even if they include different auxiliary nodes.

\subsection*{M.4 The Independent Extraction
Experiment}\label{sec-independent-extraction}
\addcontentsline{toc}{subsection}{M.4 The Independent Extraction
Experiment}

The manual extractions by Johannes and Jelena Meyer provided invaluable
data. Working independently on the same texts, their extractions
revealed:

\textbf{High Structural Agreement} (roughly 85\% overlap on main causal
relationships): Both identified the same primary causal chains, though
they sometimes decomposed them differently.

\textbf{Moderate Node Agreement} (about 70\% overlap when accounting for
semantic similarity): Different naming conventions and granularity
choices created surface disagreements masking underlying consensus.

\textbf{Low Probability Agreement} (correlation of \textasciitilde0.6
between probability estimates): Even when agreeing on structure,
numerical estimates varied substantially, confirming that probability
extraction faces inherent subjectivity.

\textbf{Consistent Ambiguity Points}: Both extractors struggled with the
same passages, suggesting certain argument features inherently resist
clean extraction.

\subsection*{M.5 Validation Methodology
Development}\label{sec-validation-methodology}
\addcontentsline{toc}{subsection}{M.5 Validation Methodology
Development}

Through iterative refinement, we developed a multi-faceted validation
approach:

\textbf{Component-Level Testing}
...
\subsection*{M.6 What We Measured and What We
Learned}\label{sec-measurement-learning}
\addcontentsline{toc}{subsection}{M.6 What We Measured and What We
Learned}

Our validation efforts yielded both quantitative metrics and qualitative
insights:
...
\item
  Performance degradation of \textasciitilde20\% on informal blog posts
  versus academic papers
\item
  Probability extraction accuracy within ±0.15 of expert estimates in
  70\% of cases
\item
  Computational efficiency enabling 50-100x speedup over manual
  extraction
\end{itemize}
...
\tightlist
\item
  Failures cluster around implicit arguments requiring extensive
  background knowledge
\item
  System excels at extracting explicit causal claims but struggles with
  implied relationships
\item
  Probability extraction benefits enormously from even minimal human
  review
\item
  Visualization quality matters as much as extraction accuracy for
  practical utility
\end{itemize}

\subsection*{M.7 The Validation Stack We Wish We
Had}\label{sec-validation-wishlist}
\addcontentsline{toc}{subsection}{M.7 The Validation Stack We Wish We
Had}

Time constraints prevented implementing our full validation vision.
...
\textbf{Automated Testing Infrastructure}:
...
\textbf{Human Evaluation Protocols}:
...
\item
  Sensitivity analysis of downstream conclusions to extraction
  variations
\item
  Causal analysis of what drives extraction quality
\end{itemize}

\subsection*{M.8 Philosophical Reflections on
Validation}\label{sec-validation-philosophy}
\addcontentsline{toc}{subsection}{M.8 Philosophical Reflections on
Validation}

The validation journey forced confrontation with deep questions about
knowledge representation:

\textbf{The Map-Territory Distinction}: Extracted models are maps of
argument territories. Like geographical maps, they can be useful without
being perfect mirrors of reality. Validation should measure usefulness,
not just accuracy.

\textbf{The Observer Effect}: Knowing they'll be extracted might change
how authors write arguments. Future systems may need to account for
strategic adaptation.

\textbf{The Formalization Paradox}: The act of formalization changes
what is formalized. Making arguments explicit reveals ambiguities that
informal presentation concealed. Is this bug or feature?

\textbf{The Validation Recursion}: Who validates the validators? As
these systems influence high-stakes decisions, the validation process
itself requires scrutiny and perhaps formal modeling.

\subsection*{M.9 Toward Trustworthy
Extraction}\label{sec-trustworthy-extraction}
\addcontentsline{toc}{subsection}{M.9 Toward Trustworthy Extraction}

Perfect validation may be impossible, but trustworthy extraction remains
achievable through:

\textbf{Transparent Limitations}: Clear documentation of what the system
can and cannot do reliably

\textbf{Uncertainty Propagation}: Carrying extraction uncertainty
through to final analyses

\textbf{Human Partnership}: Designing workflows that leverage human
judgment where machines struggle

\textbf{Continuous Improvement}: Treating validation as an ongoing
process, not a one-time certification

\textbf{Community Standards}: Developing shared benchmarks and
evaluation protocols across research groups

The goal isn't creating systems we trust blindly but systems that earn
trust through transparent operation, acknowledged limitations, and
demonstrable utility. In this light, validation becomes less about
proving perfection and more about building justified confidence.

\chapter*{Appendix N: Bucknall Case Study - Near-Term AI as Existential
Risk Factor}\label{sec-appendix-bucknall}
\addcontentsline{toc}{chapter}{Appendix N: Bucknall Case Study -
Near-Term AI as Existential Risk Factor}

\markboth{Appendix N: Bucknall Case Study - Near-Term AI as Existential
Risk Factor}{Appendix N: Bucknall Case Study - Near-Term AI as
Existential Risk Factor}

\subsection*{N.1 Introduction to the Bucknall
Extraction}\label{sec-bucknall-intro}
\addcontentsline{toc}{subsection}{N.1 Introduction to the Bucknall
Extraction}

The manual extraction of Bucknall and Dori-Hacohen's ``Current and
Near-Term AI as a Potential Existential Risk Factor''
\textcite{bucknall2022} serves as more than a validation exercise---it
provides a window into how experts navigate the extraction process and
where human judgment proves essential.

This paper makes an ideal test case for several reasons. First, it
explicitly discusses multiple causal pathways from near-term AI to
existential risk, providing rich material for extraction. Second, it
bridges technical and policy concerns, testing the system's ability to
handle interdisciplinary arguments. Third, its relatively recent
publication (2022) means it engages with current AI capabilities rather
than speculative future systems.

The manual extraction process, conducted independently by Johannes Meyer
and Jelena Meyer, revealed both the possibilities and limitations of
formalizing complex sociotechnical arguments.

\subsection*{N.2 The Extracted Structure}\label{sec-bucknall-structure}
\addcontentsline{toc}{subsection}{N.2 The Extracted Structure}

The manual extraction revealed a hierarchical argument with existential
risk as the root outcome, influenced by multiple pathways:

\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{[Existential\_Risk]: }\NormalTok{Increase in existential risks for humanity. \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}

\SpecialStringTok{{-} }\CommentTok{[}\OtherTok{Unaligned\_AGI\_Risk}\CommentTok{]}\NormalTok{: Unaligned artificial general intelligence causes existential risk. \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{  {-} }\CommentTok{[}\OtherTok{State{-}State\_Relations}\CommentTok{]}
\SpecialStringTok{{-} }\CommentTok{[}\OtherTok{Near\_term\_AI}\CommentTok{]}\NormalTok{: Even if not unaligned AGI, near term AI can act as intermediate risk factor. \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{  {-} }\CommentTok{[}\OtherTok{State{-}State\_Relations}\CommentTok{]}\NormalTok{: AI arms race dynamic inhibits international coordination, diverting resources from other pressing issues \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{    {-} }\CommentTok{[}\OtherTok{Cybersecurity}\CommentTok{]}\NormalTok{: Probably enhances Cyber{-}Attack{-}Offense, may intensify cyber warfare. \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{  {-} }\CommentTok{[}\OtherTok{State{-}Corporation\_Relations}\CommentTok{]}\NormalTok{: Corporations have a lot of power and might have misaligned goals with society \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{  {-} }\CommentTok{[}\OtherTok{Stable\_Repressive\_Regime}\CommentTok{]}\NormalTok{: More repressive instruments, possibility of stable repressive regime. \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{    {-} }\CommentTok{[}\OtherTok{State{-}Citizen\_Relations}\CommentTok{]}\NormalTok{: AI helps regime monitor citizens \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{  {-} }\CommentTok{[}\OtherTok{Compromised\_Political\_Decision\_Making}\CommentTok{]}\NormalTok{: AI can compromise political decision making. \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{    {-} }\CommentTok{[}\OtherTok{Social\_media\_and\_Recommender\_Systems}\CommentTok{]}\NormalTok{: Influence of AI in social media on public opinion. \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{{-} }\CommentTok{[}\OtherTok{Nuclear}\CommentTok{]}\NormalTok{: Probability that nuclear conflict escalates to end civilisation. \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{  {-} }\CommentTok{[}\OtherTok{Compromised\_Political\_Decision\_Making}\CommentTok{]}
\SpecialStringTok{{-} }\CommentTok{[}\OtherTok{Biological}\CommentTok{]}\NormalTok{: Probability that a natural or engineered pandemic poses existential risks. \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{  {-} }\CommentTok{[}\OtherTok{Compromised\_Political\_Decision\_Making}\CommentTok{]}
\SpecialStringTok{  {-} }\CommentTok{[}\OtherTok{Social\_media\_and\_Recommender\_Systems}\CommentTok{]}
\SpecialStringTok{{-} }\CommentTok{[}\OtherTok{Natural}\CommentTok{]}\NormalTok{: Non{-}human caused existential risks, seem unrelated with AI. \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{{-} }\CommentTok{[}\OtherTok{Environmental}\CommentTok{]}\NormalTok{: Probability of climate catastrophe. \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{  {-} }\CommentTok{[}\OtherTok{Compromised\_Political\_Decision\_Making}\CommentTok{]}
\SpecialStringTok{  {-} }\CommentTok{[}\OtherTok{AI\_resource\_consumption}\CommentTok{]}\NormalTok{: Current AI models consume large amounts of energy having environmental impacts. \{"instantiations": }\CommentTok{[}\OtherTok{"TRUE", "FALSE"}\CommentTok{]}\NormalTok{\}}
\SpecialStringTok{  {-} }\CommentTok{[}\OtherTok{Social\_media\_and\_Recommender\_Systems}\CommentTok{]}
\end{Highlighting}
\end{Shaded}
...
\subsection*{N.3 Key Insights from Manual
Extraction}\label{sec-bucknall-insights}
\addcontentsline{toc}{subsection}{N.3 Key Insights from Manual
Extraction}

The extraction process revealed several important patterns:

\textbf{Multi-Path Risk Architecture}: Unlike simpler models with linear
causal chains, Bucknall's argument presents existential risk as emerging
from multiple, interacting pathways. Near-term AI doesn't directly cause
existential catastrophe but acts as a risk multiplier across various
domains.

\textbf{Institutional Intermediaries}: The model highlights how AI
affects existential risk through institutional mechanisms---state
relations, corporate power, political decision-making. This sociological
sophistication challenges purely technical risk models.

\textbf{Reused Components}: Several nodes (like
``Compromised\_Political\_Decision\_Making'') influence multiple risk
pathways. This reuse reflects how certain AI impacts create systemic
vulnerabilities rather than isolated risks.

\textbf{Implicit Probabilities}: While the structure emerged clearly,
the paper provides few explicit probability estimates. Extractors had to
infer likely ranges from qualitative language like ``probably enhances''
and ``may intensify.''

\subsection*{N.4 Extraction Challenges and
Decisions}\label{sec-bucknall-challenges}
\addcontentsline{toc}{subsection}{N.4 Extraction Challenges and
Decisions}

The manual extraction process faced several decision points that
illuminate the broader challenge:

\textbf{Granularity Choices}: How finely should arguments be decomposed?
The extractors chose to keep ``State-State\_Relations'' as a single node
rather than breaking it into specific mechanisms, judging that further
decomposition would lose coherence.

\textbf{Temporal Ambiguity}: The paper discusses both current impacts
and future possibilities without always clearly distinguishing.
Extractors had to decide whether to model these as separate nodes or
probability variations.

\textbf{Causal vs.~Definitional}: Some relationships blur causation and
definition. Is ``AI helps regime monitor citizens'' a cause of
``Stable\_Repressive\_Regime'' or part of what defines it? Such
philosophical questions arose repeatedly.

\textbf{Background Assumptions}: The paper assumes familiarity with
existential risk frameworks. Extractors had to decide how much implicit
framework to make explicit in the model.

\subsection*{N.5 Validation Insights}\label{sec-bucknall-validation}
\addcontentsline{toc}{subsection}{N.5 Validation Insights}

Comparing the independent extractions revealed:

\textbf{Structural Convergence}: Both extractors identified the same
major causal pathways and risk factors, suggesting the paper's argument
structure is reasonably unambiguous.

\textbf{Naming Variations}: Different choices in node naming
(``State-Corporation\_Relations'' vs.~``Corporate\_Power'') created
surface disagreements while representing the same concepts.

\textbf{Probability Divergence}: Where extractors attempted to quantify
relationships, estimates varied by 20-30 percentage points, confirming
that probability extraction faces inherent subjectivity.

\textbf{Consistent Struggle Points}: Both extractors found certain
passages challenging, particularly where the paper discusses complex
feedback loops between AI development and geopolitical dynamics.

\subsection*{N.6 Implications for Automated
Extraction}\label{sec-bucknall-implications}
\addcontentsline{toc}{subsection}{N.6 Implications for Automated
Extraction}

This manual extraction exercise validates several AMTAIR design
decisions:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Two-Stage Architecture}: The clear separation between
  structural and probability extraction in human practice supports the
  system's two-stage approach.
\item
  \textbf{Human-in-the-Loop}: Even expert human extractors made judgment
  calls requiring domain knowledge, confirming that full automation
  remains premature for high-stakes applications.
\item
  \textbf{Variation Acceptance}: The legitimate differences between
  expert extractions suggest systems should support multiple
  interpretations rather than seeking single ``correct'' outputs.
\item
  \textbf{Metadata Importance}: Preserving descriptions and context
  proves essential for interpreting formalized models, validating the
  BayesDown format's hybrid approach.
\end{enumerate}

\subsection*{N.7 From Manual to
Automated}\label{sec-bucknall-automation}
\addcontentsline{toc}{subsection}{N.7 From Manual to Automated}

How would AMTAIR handle this same extraction task? Based on similar
complexity arguments:

\begin{itemize}
\item
  \textbf{Structure Extraction}: The system would likely capture the
  main risk pathways successfully, though it might struggle with the
  nuanced institutional relationships.
\item
  \textbf{Node Identification}: Explicit risk factors would extract
  cleanly, but implicit mechanisms might be missed or mischaracterized.
\item
  \textbf{Probability Assignment}: Without explicit estimates in the
  text, the system would rely on linguistic mappings, likely producing
  reasonable but imprecise values.
\item
  \textbf{Processing Time}: What took human experts 4-6 hours would
  complete in under 2 minutes, enabling analysis at scale despite lower
  per-document precision.
\end{itemize}

\subsection*{N.8 The Value Proposition
Crystallized}\label{sec-bucknall-value}
\addcontentsline{toc}{subsection}{N.8 The Value Proposition
Crystallized}

The Bucknall case study crystallizes AMTAIR's value proposition. Perfect
extraction remains elusive even for human experts. But rapid,
good-enough extraction that captures primary arguments and enables
systematic comparison across many documents provides genuine value. The
goal isn't replacing human judgment but augmenting human capacity to
synthesize complex arguments about existential risk.

In a world where relevant arguments proliferate faster than experts can
read them, tools that extract 80\% of the structure 100 times faster
than humans transform from nice-to-have to necessary-for-coordination.
The Bucknall extraction shows both why this transformation matters and
